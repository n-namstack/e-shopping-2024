...
const fetchShops = async (user_id, setShops, setLoading) => {
  try {
    const response = await axios.get('http://192.168.178.87:8000/api/shops', {
...
const getShopInfo = async (user_id, shop_uuid, setShop) => {
  try {
    const response = await axios.get(
      'http://192.168.178.87:8000/api/shop-info',
      {
...
const createShop = async (shopInfo) => {
  try {
    const res = await axios.post('http://192.168.178.87:8000/api/create-shop', {
