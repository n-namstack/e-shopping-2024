...
const Analytics = ({ navigation }) => {
  ...
  const loadAnalytics = async () => {
    try {
      ...
      if (ordersError) {
        console.error(
          "Error loading orders for metrics calculation:",
          ordersError.message
        );
      }
      ...
      if (messagesError) {
        console.error(
          "Error loading messages for response rate calculation:",
      ...
      if (productsData && productsData.length > 0) {
        ...
        console.log(
          `Top products data prepared: ${topProductsData.length} products`
        );
        console.log(
          `Category performance data prepared: ${categoryData.length} categories`
        );
      }
      ...
      if (messagesData && messagesData.length > 0) {
        ...
        const respondedMessages = messagesData.filter(
          (msg) => msg.response_time_minutes != null
        ).length;
        responseRate = respondedMessages / totalMessages;

        // Calculate average response time for responded messages
        const respondedMessagesData = messagesData.filter(
          (msg) => msg.response_time_minutes != null
        );
        const avgResponseTime =
          respondedMessagesData.length > 0
            ? respondedMessagesData.reduce(
                (sum, msg) => sum + msg.response_time_minutes,
                0
              ) / respondedMessagesData.length
            : 0;

        console.log(
          `Response metrics: Rate=${responseRate.toFixed(
            2
          )}, Avg Time=${avgResponseTime.toFixed(2)} minutes`
        );
      }
        ...
        const deliveredOrders = ordersData.filter(
          (order) =>
            ...
            new Date(order.delivery_date) <=
              new Date(order.expected_delivery_date || order.delivery_date)
        ).length;

        const totalCompletedOrders = ordersData.filter(
          (order) => order.status === "completed"
        ).length;
        deliverySuccessRate =
          totalCompletedOrders > 0 ? deliveredOrders / totalCompletedOrders : 0;
      ...
      const aggregatedStats = statsData.reduce(
        (acc, stat) => ({
          ...
          processing_orders:
            acc.processing_orders + (stat.processing_orders || 0),
          total_revenue:
            acc.total_revenue + parseFloat(stat.total_revenue || 0),
          average_order_value:
            acc.average_order_value + parseFloat(stat.average_order_value || 0),
          total_customers: acc.total_customers + (stat.total_customers || 0),
          followers_count: acc.followers_count + (stat.followers_count || 0),
          average_rating:
            acc.average_rating +
            parseFloat(stat.average_rating || 0) / statsData.length,
          monthly_growth_rate:
            acc.monthly_growth_rate +
            parseFloat(stat.monthly_growth_rate || 0) / statsData.length,
          response_rate:
            acc.response_rate +
            parseFloat(stat.response_rate || 0) / statsData.length,
          delivery_success_rate:
            acc.delivery_success_rate +
            parseFloat(stat.delivery_success_rate || 0) / statsData.length,
      ...
      const performanceData = {
        data: [
          Math.min(
            1,
            Math.max(0.1, aggregatedStats.completed_orders / (totalOrders || 1))
          ),
      ...
      setStats({
        ...
        monthlyGrowthData,
        ratingDistribution,
        total_products: statsData.reduce(
          (sum, stat) => sum + (stat.total_products || 0),
          0
        ),
      });
      ...
      try {
        ...
        if (recentOrdersError) {
          console.error(
            "Error loading recent orders for customer activity:",
          ...
          const reorderedDayCounts = [
            ...dayOfWeekCounts.slice(1),
            dayOfWeekCounts[0],
          ];
        ...
        const { data: customerGrowthData, error: customerGrowthError } =
          await supabase
            .from("orders")
            .select("created_at, user_id")
            .in("shop_id", shopIds)
        ...
        if (customerGrowthError) {
          console.error(
            "Error loading customer growth data:",
            customerGrowthError.message
          );
            ...
            <>
                    ...
                    <View
                      style={[
                        styles.bar,
                        {
                          width: `${Math.min(
                            100,
                            (stats.completed / (stats.totalOrders || 1)) * 100
                          )}%`,
                    ...
                    <View
                      style={[
                        styles.bar,
                        {
                          width: `${Math.min(
                            100,
                            (stats.pending / (stats.totalOrders || 1)) * 100
                          )}%`,
                    ...
                    <View
                      style={[
                        styles.bar,
                        {
                          width: `${Math.min(
                            100,
                            (stats.cancelled / (stats.totalOrders || 1)) * 100
                          )}%`,
                    ...
                    <View
                      style={[
                        styles.bar,
                        {
                          width: `${Math.min(
                            100,
                            (stats.processing / (stats.totalOrders || 1)) * 100
                          )}%`,
                      ...
                      <View style={styles.pieChartContainer}>
                        {[
                          ...
                          },
                          {
                            ...
                            color: "#007AFF",
                            startDeg:
                              (stats.completed / stats.totalOrders) * 360,
                          },
                          {
                            value: stats.cancelled,
                            color: "#FF4757",
              ...
              <View style={styles.customPerformanceCard}>
                ...
                <Text style={styles.metricsSubtitle}>
                  Based on actual order and customer data
                </Text>
                  ...
                  {[
                    {
                      ...
                      description:
                        "Percentage of total orders that have been completed",
                      value:
                        stats.totalOrders > 0
                          ? Math.min(
                              100,
                    ...
                    },
                    {
                      ...
                      value: Math.min(
                        100,
                        Math.round(parseFloat(stats.responseRate || 0) * 100)
                      ),
                      color: "#FF4757",
                    },
                    {
                      ...
                      description:
                        "Percentage of orders delivered on or before expected date",
                      value: Math.min(
                        100,
                        Math.round(
                          parseFloat(stats.deliverySuccessRate || 0) * 100
                        )
                      ),
                      color: "#FFC107",
                    },
            ...
            <>
              ...
              <View style={styles.summaryCard}>
                <View style={styles.summaryHeader}>
                  <View>
                    ...
                    <Text style={styles.summarySubtitle}>
                      Key metrics and revenue breakdown
                    </Text>
                  </View>
                ...
                <View style={styles.metricRow}>
                  ...
                  <View style={styles.metricCard}>
                    <Text style={styles.metricValue}>
                      {stats.ratingDistribution?.datasets?.[0]?.data?.reduce(
                        (a, b) => a + b,
                        0
                      ) || 0}
                    </Text>
                    ...
                    <View style={styles.topProductsList}>
                      {stats.topProductsPieData.map((product, index) => {
                        ...
                        const maxValue = Math.max(
                          ...stats.topProductsPieData.map((p) => p.population)
                        );
                        ...
                        const percentage =
                          maxValue > 0
                            ? Math.max(
                                10,
                                (product.population / maxValue) * 100
                              )
                            : 0;
                          ...
                          <View key={index} style={styles.topProductItem}>
                            <View style={styles.productNameRow}>
                              ...
                              <Text style={styles.productValue}>
                                {formatCurrency(product.population)}
                              </Text>
                            </View>
                  ...
                  <>
                    ...
                    <View style={styles.categoryBreakdown}>
                      {stats.categoryPieData.map((category, index) => {
                        const totalValue = stats.categoryPieData.reduce(
                          (sum, cat) => sum + cat.population,
                          0
                        );
                        const percentage =
                          totalValue > 0
                            ? Math.round(
                                (category.population / totalValue) * 100
                              )
                            : 0;
              ...
              <View style={styles.chartCard}>
                ...
                stats.ratingDistribution.datasets[0].data.some(
                  (value) => value > 0
                ) ? (
                  <>
                    <Text style={styles.chartSubtitle}>
                    ...
                    <View style={styles.starRatingsContainer}>
                      {stats.ratingDistribution.labels.map((label, index) => {
                        const count =
                          stats.ratingDistribution.datasets[0].data[index];
                        const total =
                          stats.ratingDistribution.datasets[0].data.reduce(
                            (a, b) => a + b,
                            0
                        ...
                        return (
                          <View key={index} style={styles.starRatingItem}>
                            ...
                            <Text style={styles.starRatingPercentage}>
                              {percentage}%
                            </Text>
                          </View>
                        );
          ...
          {activeTab === "customers" && (
            <>
              ...
              <View style={styles.summaryCard}>
                ...
                <View style={styles.customerMetrics}>
                  <View style={styles.customerMetric}>
                    ...
                    <Text style={styles.customerMetricLabel}>
                      Total Customers
                    </Text>
                  </View>
                  <View style={styles.customerMetric}>
              ...
              <View style={styles.chartCard}>
                ...
                <View style={styles.chartInfoRow}>
                  ...
                  <View style={styles.chartInfoItem}>
                    ...
                    <Text style={styles.chartInfoValue}>
                      {stats.customerActivityData?.datasets?.[0]?.data
                        ? stats.customerActivityData.datasets[0].data.reduce(
                            (a, b) => a + b,
                            0
                          )
                        : 0}
              ...
              <View style={styles.chartCard}>
                ...
                {stats.customerGrowthData &&
                  stats.customerGrowthData.datasets[0].data.length > 1 && (
                    <View style={styles.chartInfoRow}>
                      <View style={styles.chartInfoItem}>
                        ...
                        <Text style={styles.chartInfoValue}>
                          {(() => {
                            const data =
                              stats.customerGrowthData.datasets[0].data;
                      ...
                      <View style={styles.chartInfoItem}>
                        ...
                        <Text style={styles.chartInfoValue}>
                          {
                            stats.customerGrowthData.datasets[0].data[
                              stats.customerGrowthData.datasets[0].data.length -
                                1
                            ]
                          }{" "}
                          customers
              ...
              <View style={styles.chartCard}>
                ...
                {stats.customerHeatmapData &&
                stats.customerHeatmapData.length > 0 ? (
                  <ContributionGraph
                ...
                <Text style={styles.chartCaption}>
                  Customer activity over last 3 months
                </Text>
              ...
              <View style={styles.summaryCard}>
                ...
                <View style={styles.growthMetric}>
                  ...
                  <Text style={styles.growthValue}>
                    {stats.monthlyGrowthRate?.toFixed(2) || "0.00"}%
                  </Text>
                </View>
              </View>
