...
const Analytics = ({ navigation }) => {
  ...
  const loadAnalyticsData = async (initialShopIds = null) => {
    try {
      ...
      const processed = await processAnalyticsData({ orders: ordersData, stats: [], products: productsData, reviews: reviewsData, shopIds });
  ...
  const generateMonthlyRevenue = (orders) => {
    ...
    orders.forEach((o) => {
      if (o.payment_status === "paid" || o.status === "completed" || o.status === "delivered") {
  ...
  const formatCurrency = (v) => `N$${new Intl.NumberFormat("en-NA",{minimumFractionDigits:0,maximumFractionDigits:0}).format(v)}`;
